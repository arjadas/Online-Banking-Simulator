describe('Sign Up Page', () => {
    ...
    it('renders all elements correctly', () => {
        ...
        cy.get('input[name="confirm_password"]').should('have.attr', 'placeholder', 'Confirm Password');
        cy.contains('button', 'Sign up').should('be.visible');
        cy.contains('Go back to Login').should('have.attr', 'href', '/login');
    });
    ...
    it('shows error if passwords do not match', () => {
        ...
        cy.get('input[name="last_name"]').type('Account');
        cy.get('input[name="email"]').type('test@email.com');
        cy.get('input[name="password"]').type('correctpassword123');
        cy.get('input[name="confirm_password"]').type('correctpassword456');
        cy.get('button[type="submit"]').click();
    ...
    it('shows error if email already in use', () => {
        ...
        cy.get('input[name="confirm_password"]').type('correctpassword');
        cy.get('button[type="submit"]').click();
        
        cy.contains('An Account With This Email Already Exist.').should('be.visible');
    });
    

    // // Currently not working yet: Same email error
    // it('submits form successfully when all fields are filled and valid', () => {
    //     cy.get('input[name="first_name"]').type('Test');
    //     cy.get('input[name="last_name"]').type('Account');
    //     cy.get('input[name="email"]').type('test@email.com');
    //     cy.get('input[name="password"]').type('correctpassword');
    ...
    it('navigates to the Login page when clicking "Go back to Login"', () => {
        cy.contains('a', 'Go back to Login').click();
